...
const toPlayableVideoUrl = (rawUrl?: string) => {
  ...
  if (url.includes("/res.cloudinary.com/") && url.includes("/video/upload/")) {
    if (
      url.includes("/video/upload/f_mp4,") ||
      url.includes("/video/upload/f_mp4/")
    ) {
      return url;
    }
...
const TrendingScreen = () => {
  ...
  const { data: t } = useTranslateTexts({
    texts: [
      ...
      "because you have not shared the most recent UBlast. ",
    ],
    targetLang: language,
  ...
  const { data: offersData, refetch: refetchOffers } = useGetUblastOffers({
    enabled: !!user?.token && selectedTab === "active",
    fetchAll: true,
    limit: 50,
    maxPages: 20,
  ...
  const getNotEligibleMessage = (blockedUntil?: string | null) => {
    ...
    if (blockedUntilText) {
      return `${tx(42, "You are currently blocked from submitting to UBlast until")} ${blockedUntilText} ${tx(43, "because you have not shared the most recent UBlast. ")}`;
    }
  ...
  const sharedByUblastIdRef = useRef(sharedByUblastId);
  useEffect(() => {
    sharedByUblastIdRef.current = sharedByUblastId;
  }, [sharedByUblastId]);
  ...
  const offersByUblastId = useMemo(() => {
    ...
    offers.forEach((offer: any) => {
      const ublastId =
        offer?.ublastId?._id || offer?.ublastId?.id || offer?.ublastId || null;
      if (!ublastId) return;
  ...
  const searchedActiveUblasts = useMemo(() => {
    ...
    return activeUblasts.filter((item: any) => {
      ...
      if (
        ublastTitle.includes(normalizedSearch) ||
        ublastContent.includes(normalizedSearch)
      )
        return true;
  ...
  const getItemMediaInfo = useCallback(
    (item: any) => {
      if (selectedTab === "active") {
        const sharedPost = sharedByUblastIdRef.current.get(String(item?._id));
        if (sharedPost) {
          return {
            id: String(sharedPost?._id || ""),
            mediaType: String(sharedPost?.mediaType || ""),
          };
        }
      }
      return {
  ...
  const onViewableItemsChanged = useRef(
    ({ viewableItems }: { viewableItems: ViewToken[] }) => {
      ...
      viewableItems.forEach((viewable) => {
        const info = getItemMediaInfo(viewable?.item);
        const id = info.id;
        if (id) nextIds.add(id);
        if (!nextActiveVideoId && info.mediaType === "video" && id) {
          nextActiveVideoId = id;
        }
      });
      ...
      setActiveVideoId((prev) =>
        prev === nextActiveVideoId ? prev : nextActiveVideoId,
      );
    },
  ).current;
  ...
  const UblastCard = ({
    ...
    type ExternalShareTarget =
      ...
      | "snapchat"
      | "spotify";
    const { mutateAsync: shareUblast, isPending: isSharePending } =
      useShareUblast();
    const { mutateAsync: checkoutOffer, isPending: isCheckoutPending } =
      useCheckoutUblastOffer();
    const { mutateAsync: cancelOffer, isPending: isCancelPending } =
      useCancelUblastOffer();
    ...
    const proxyMediaUrl = toProxyMediaUrl(mediaUrlRaw);
    const displayMediaUrl = proxyMediaUrl || mediaUrlRaw;
    const mediaType = item?.mediaType;
    const shouldUsePlayer = mediaType === "video" || mediaType === "audio";
    const mediaPlaybackUrl =
      mediaType === "video"
        ? toPlayableVideoUrl(displayMediaUrl)
        : displayMediaUrl;
    const player = useVideoPlayer(
      shouldUsePlayer ? mediaPlaybackUrl : "",
      (p) => {
        p.loop = true;
      },
    );
    ...
    useEffect(() => {
      ...
      if (mediaType === "video") {
        if (isVisible && isFocused && isActiveVideo) {
          player.play();
        } else {
          player.pause();
        }
    ...
    }, [isVisible, isFocused, mediaType, player, isActiveVideo]);

    const handleShare = async (
      shareType: "feed" | "story",
      external?: ExternalShareTarget,
    ) => {
      if (!item?._id) return;
      if (!canShare) {
        Toast.show({
      ...
      setIsShareBusy(true);
      try {
        const eligibilitySnapshot = ((await refetchEligibility())?.data ??
      ...
      try {
        ...
        if (checkoutUrl) {
          ...
          if (result.type === "success" && result.url) {
            ...
            const payment = Array.isArray(qp.payment)
              ? qp.payment[0]
              : qp.payment;
            if (payment === "success") {
              Toast.show({
    ...
    return (
      <View className="bg-[#F0F2F5] dark:bg-[#FFFFFF0D] rounded-3xl mx-5 mt-4 overflow-hidden">
        ...
        {mediaType === "image" && displayMediaUrl ? (
          <Image
          ...
          isVisible && isFocused && isActiveVideo ? (
            <VideoView
              style={{ width: "100%", height: 220 }}
              player={player}
          ...
          <View className="flex-row gap-4">
            ...
            {showOfferActionButtons ? (
              ...
              <TouchableOpacity
                onPress={async () => {
                  if (!item?._id) return;
                  const eligibilitySnapshot = ((await refetchEligibility())
                    ?.data ?? eligibilityData) as any;
                  if (eligibilitySnapshot?.eligible === false) {
                    Toast.show({
                      type: "error",
                      text1: tx(3, "Not Eligible"),
                      text2: getNotEligibleMessage(
                        eligibilitySnapshot?.blockedUntil,
                      ),
                    });
                    return;
                  }
              ...
              >
                <Ionicons
                  ...
                  color={isLightTheme ? "black" : "white"}
                  style={{
                    opacity: hasShared || isBusy || !canShare ? 0.5 : 1,
                  }}
                />
              </TouchableOpacity>
            )}
      ...
      <SafeAreaView className="flex-1 mt-2.5" edges={["top", "left", "right"]}>
        ...
        >
          ...
          <FlatList
            data={
              selectedTab === "active"
                ? searchedActiveUblasts
                : searchedFilteredPosts
            }
            renderItem={({ item }) => {
              const itemMediaInfo = getItemMediaInfo(item);
              const visibleNow =
                isFocused && visibleItemIds.has(itemMediaInfo.id);
              if (selectedTab === "active") {
                const sharedPost = sharedByUblastId.get(String(item?._id));
                if (sharedPost) {
                  return (
                    <PostCard
                      ...
                      isActiveVideo={
                        String(sharedPost?._id || "") === activeVideoId
                      }
                      disableShare={
                        !isEligible &&
                        (Boolean((sharedPost as any)?.ublastId) ||
                          String(
                            (sharedPost as any)?.postType || "",
                          ).toLowerCase() === "ublast")
                      }
                      shareDisabledMessage={getNotEligibleMessage(
                        (eligibilityData as any)?.blockedUntil,
                      )}
                    />
                  );
                ...
                return (
                  <UblastCard
                    ...
                    isVisible={visibleNow}
                    isFocused={isFocused}
                    isActiveVideo={String(item?._id || "") === activeVideoId}
                  />
                );
              }
              return (
                <PostCard
                  ...
                  disableShare={
                    ...
                    (Boolean((item as any)?.ublastId) ||
                      String((item as any)?.postType || "").toLowerCase() ===
                        "ublast")
                  }
                  shareDisabledMessage={getNotEligibleMessage(
                    (eligibilityData as any)?.blockedUntil,
